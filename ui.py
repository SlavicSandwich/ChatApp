...
class ChatApp:
    ...
    def win_draw_chatbox(self):
        ...
        temp_lines_offset = sum([self.calc_lines_needed(len(msg), chatbox_width) for msg in self.chatbox_messages])
        if temp_lines_offset <= chatbox_height:
            self.shows_first_message = True
            cursor_y = 0
            for msg in self.chatbox_messages:
                try:
                    complete_message = str(msg)
            ...
            for i in range(min(chatbox_height, len(self.chatbox_messages))):
                try:
                    ...
                    total_lines_offset += self.calc_lines_needed(len(recent_message), chatbox_width)
                    complete_message = str(recent_message)
    ...
    def new_message(self, sender_name, message):
        object = ChatMessage(sender_name, message)
